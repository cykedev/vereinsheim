---
id: ringwerk-code-conventions
type: guide
title: "Ringwerk — Code Conventions – Ringwerk"
aliases: ["Code Conventions – Ringwerk"]
keywords: [ringwerk, code, conventions, ringwerk]
part_of: ["[[ringwerk]]"]
---

**TL;DR** Verbindlich gleichrangig mit `docs/technical.md`.
...
- **Wertungszahlen nie mit `===` vergleichen**: Korrigierte Teiler (`teiler × faktor`) und Summen von
  Nachkommaresten sind ungerundete Fließkommawerte (8,2 × 1,5 = 12.299999999999999 ≠ 12,3). Gleichheit
  über `sameScore` (`lib/scoring/sharedRanks.ts`) — und zwar im Sortier-Comparator **und** in der
  Gleichstandsprüfung dieselbe, sonst liegen Gleichstände nicht nebeneinander und ein nachgelagerter
  Tiebreak wird übersprungen. Gepinnt in `sharedRanks.test.ts` und `sortSeasonStandings.test.ts`.
